...
urlpatterns = [
    ...
    url(r'^$', cache_page(60*60*24)(views.index), name='home'),  # Cache time in seconds - so this view will be cached for 24 hours.
    url(r'^about/$', cache_page(60*60*24)(views.about), name='about'),
    url(r'^tutorial/$', cache_page(60*60*24)(views.tutorial), name='tutorial'),    
    url(r'^drivers/$', cache_page(60*60*24)(views.drivers), name='drivers'),    
    url(r'^targets/$', cache_page(60*60*24)(views.targets), name='targets'),
    url(r'^tissues/$', cache_page(60*60*24)(views.tissues), name='tissues'),
    url(r'^studies/$', cache_page(60*60*24)(views.studies), name='studies'),    
    url(r'^faq/$', cache_page(60*60*24)(views.faq), name='faq'),        
    url(r'^contact/$', cache_page(60*60*24)(views.contact), name='contact'),

    url(r'^study/(?P<study_pmid>[0-9A-Za-z]+)/$', cache_page(60*60*24)(views.show_study), name='show_study'), # pmid could be 'Pending0001'
    ...
    url(r'^get_dependencies/(?P<search_by>(?:mysearchby|driver|target))/(?P<entrez_id>[0-9a-z]+)/(?P<histotype_name>[0-9A-Za-z\_]+)/(?P<study_pmid>[0-9A-Za-z\_]+)/$', cache_page(60*60*24)(views.get_dependencies), name='get_dependencies'),
    ...
    url(r'^download_csv/(?P<delim_type>(?:csv|tsv|xlsx))/(?P<search_by>(?:mysearchby|driver|target))/(?P<entrez_id>[0-9a-z]+)/(?P<histotype_name>[0-9A-Za-z\_]+)/(?P<study_pmid>[0-9A-Za-z\_]+)/$', cache_page(60*60*24)(views.download_dependencies_as_csv_file), name='download_csv'), # \_ needed to match ALL_STUDIES and ALL_HISTOTYPES

    url(r'get_boxplot/(?P<dataformat>(?:myformat|csvplot|jsonplot|jsonplotandgene|csv|download))/(?P<driver_name>[0-9A-Za-z\-_\.]+)/(?P<target_name>[0-9A-Za-z\-_\.]+)/(?P<histotype_name>[0-9A-Za-z\_]+)/(?P<study_pmid>[0-9A-Za-z\_]+)/$', cache_page(60*60*24)(views.get_boxplot), name='get_boxplot'),
