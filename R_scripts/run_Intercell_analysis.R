...
# NOTES:
  ...
  # (3) Wang: "mmc3" is the table used from their paper (the smaller dataset in table "mmc7" isn't used here). All AML tissue so only 'by-tissue' analksis.
  # (4) Achilles_CRISPR: Not using it at present. kinome_file: "Achilles_v3.3.8_cancergd_with_entrezids.txt" and  tissues_file: "Achilles_v3.3.8_tissues.txt"
...
if (is.null(data_set) || data_set=='') {stop(paste("ERROR: Invalid data_set: '",data_set,"' but should be 'Campbell', 'Cowley', 'Marcotte' or 'Wang_mm3'"))}

output_pancaner_results <- ( data_set=="Campbell" || data_set=="Cowley" || data_set=="Achilles_CRISPR" ) # No pan-cancer results for Marcotte (as all Breast) nor Wand_mm3 (as all AML)
